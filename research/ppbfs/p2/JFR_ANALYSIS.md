...
`p2-b0-grid30.jfr`, `p2-b0-diamond.jfr` (unit, B0, ×15 repeats), and
`runs/p2-external-v1/h3-jfr*/b0.jfr + v.jfr` (server, H3). Analysis: `scripts/analyze_jfr.py`
(leaf top-frames + inclusive class shares + allocation classes). JDK 21, `settings=profile`.

## Unit profiles (B0)

- **chain2000-s255** (strong positive; 164 samples): inclusive BFSExpander 88%,
  ProductGraphTraversalCursor 78%, FoundNodes 5.5%, HeapTrackingArrayList 6.1% (leaf
  `Itr.next`/`elementData`/`get`/`checkIndex` ≈ 4.3%). Allocations led by `Object[]` (283),
  `HeapTrackingArrayList` (42), `NodeState` (32), `Lengths` (30): buckets are top-allocated
  short-lived structure. Amdahl bound for scan removal (`DERIVED` from telemetry counts ×
  fitted `c_slot`): ≈6% of harness elapsed — the measured +17% therefore rides substantially
  on allocation-rate relief (consistent with 2.47× tracked reduction), not scans alone.
